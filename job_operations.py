...
class JQueuer_Task(celery.Task):
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        ...
        log_message = ('\non_failure: Task Id: {0} \n exp_id: {1} \t worker_id: {2} \t job_id: {3} \n retries: {4} \t start_time: {5}'
                    ).format(task_id, getExperimentID(worker_id), worker_id, self.jqueuer_job["id"], self.request.retries, self.jqueuer_job_start_time)
        logger.info(log_message)
        # send metric
        response = monitoring.job_failed(worker_id,self.jqueuer_job["id"],self.jqueuer_job_start_time)
    ...
    def on_retry(self,exc, task_id, args, kwargs, einfo):
        ...
        logger.info(log_message)
        # send metric
        response = monitoring.terminate_retried_job(worker_id, self.jqueuer_job["id"])
        if response.lower() == "stop_worker":
            container_dead = True
            pause_output = pause_container(worker_id)
    ...
    def on_success(self,retval, task_id, args, kwargs):
        ...
        log_message = ('\non_success: Task Id: {0} \n exp_id: {1} \t worker_id: {2} \t job_id: {3} \t start_time: {4}'
                    ).format(task_id, getExperimentID(worker_id), worker_id, self.jqueuer_job["id"], self.jqueuer_job_start_time)
        logger.info(log_message)
        # send metric
        response = monitoring.terminate_job(worker_id, self.jqueuer_job["id"], self.jqueuer_job_start_time)
        if response.lower() == "stop_worker":
            container_dead = True
            pause_output = pause_container(worker_id)
...
def add(self, exp_id, job_queue_id, job):
    ...
    monitoring.run_job(worker_id, job["id"])
    tasks = job["tasks"]
...
def process_list(worker_id, exp_id, job_queue_id, job, job_start_time):
    ...
    for task in job["tasks"]:
        ...
        monitoring.run_task(worker_id,job["id"],task["id"])
        ...
        output = subprocess.check_output(command)
        monitoring.terminate_task(worker_id,job["id"],task["id"],task_start_time)
...
def process_array(worker_id, exp_id, job_queue_id, job, job_start_time):
    ...
    for x in range(0, tasks["count"]):
        ...
        monitoring.run_task(worker_id, job["id"], task_id)
        command = (
            ["docker", "exec", getContainerID(worker_id)]
            + tasks["command"]
            + [str(tasks["data"])]
        ...
        output = subprocess.check_output(command)
        monitoring.terminate_task(worker_id, job["id"], task_id, task_start_time)
